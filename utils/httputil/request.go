...
// RAW 发起常规请求
func (s *Client) RAW(body ...[]byte) ([]byte, []byte, error) {
	s.init()
	defer s.close()
	s.setHeaders()
...
// FILE 发起文件请求
func (s *Client) FILE(fname string, fctt []byte) ([]byte, []byte, error) {
	s.init()
	defer s.close()
	s.setHeaders()
	s.req.SetRequestURI(s.URI)
...
// FORM 上传多键值对以及文件
func (s *Client) FORM(formList []RequestForm) ([]byte, []byte, error) {
	var (
		buf        bytes.Buffer
		bodyWriter = multipart.NewWriter(&buf)
	)
	if len(formList) == 0 {
		return nil, nil, errors.New("form request failed,no form value given")
	}
	s.init()
	defer s.close()
	s.setHeaders()
...
func (s *Client) request() ([]byte, []byte, error) {
	var err error
	if s.Timeout != 0 {
	...
	if s.resp.StatusCode() != http.StatusOK {
		return nil, nil, errors.New(fmt.Sprintf("response data code:%d", s.resp.StatusCode()))
	}
	respBody := s.resp.Body()
	cttType := s.resp.Header.ContentType()
	return respBody, cttType, nil
}
